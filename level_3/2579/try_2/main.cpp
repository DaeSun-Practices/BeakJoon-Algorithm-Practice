...
int main () {
    ...
    bool selection_array[N+1] = {false, };
    vector<pair<int, int>> stair_vector;

    for (int i = 0; i < N; i++){  
        int grade;
        cin >> grade;
        stair_vector.push_back(pair<int, int>(grade,i+1));
    }

    int total_grade = stair_vector.back().first; stair_vector.pop_back();
    selection_array[N] = true;
    selection_array[0] = true;
    ...
    for (int i=0; i<N-1; i++){
        ...
        if (stair_index<=N-1 && stair_index>=1){
